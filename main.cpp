...
//	});
	...
	server.set_http_handler<GET, POST>("/upload_multipart_form", [&n](const request& req, response& res) {
		...
		switch(state)
        {
            ...
            case data_proc_state::data_continue:
            {
                ...
                break;
            case data_proc_state::file_begin:
            {
                //req.set_continue(false); //中断文件接受
            }
                break;
            case data_proc_state::file_continue:
            {

            }
                break;
            case data_proc_state::data_end:
            {

            }
                break;
            case data_proc_state::file_end:
            {

            }
                break;
            case data_proc_state::data_all_end:
            {
                 auto multipart_text=  req.get_query_value("text");
                 std::cout<<multipart_text<<std::endl;
                 auto &files = req.get_upload_files();
                 for(auto &file: files)
                 {
                     std::cout<<"file_path=="<<file.get_file_path()<<"size=="<<file.get_file_size()<<std::endl;
                 }
                 auto file_path = files.size()==0?"":files[0].get_file_path();
