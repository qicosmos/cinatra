...
public:
  ...
  template <typename F>
  void init_ssl_context(bool ssl_enable_v3, F &&f,
    ...
    ctx_.use_private_key_file(std::move(private_key_file),
  ...
  std::string get_public_root_directory() { return public_root_path_; }
...
private:
  void start_accept(
    ...
    auto new_conn = std::make_shared<connection<Socket>>(
        io_service_pool_.get_io_service(), max_req_buf_size_,
        keep_alive_timeout_, http_handler_, static_dir_,
    ...
    set_http_handler<POST, GET>(
        ...
        [this](request &req, response &res) {
          ...
          case cinatra::data_proc_state::data_begin: {
            ...
            auto mime = req.get_mime(
                {relatice_file_name.data(), relatice_file_name.length()});
            std::wstring source_path =
                fs::absolute(relatice_file_name).filename().wstring();
            std::wstring root_path =
                fs::absolute(public_root_path_).filename().wstring();
            if (source_path.find(
                    fs::absolute(root_path).filename().wstring()) ==
                std::string::npos) {
              auto it = std::find_if(
                  relate_paths_.begin(), relate_paths_.end(),
                  [this, &relatice_file_name](auto &str) {
                    auto pos = relatice_file_name.find(str);
                    if (pos != std::string::npos) {
                      relatice_file_name = relatice_file_name.replace(
                          0, str.size(),
                          public_root_path_.substr(0, public_root_path_.size() -
                                                          1));
                    }
            ...
            auto in = std::make_shared<std::ifstream>(relatice_file_name,
                                                      std::ios_base::binary);
            if (!in->is_open()) {
              if (not_found_) {
                not_found_(req, res);
  ...
  void write_chunked_header(request &req, std::shared_ptr<std::ifstream> in,
    ...
    conn->write_chunked_header(
        std::string_view(res_content_header.data(), res_content_header.size()),
        req.is_range());
  }
