...
class coro_http_client : public std::enable_shared_from_this<coro_http_client> {
 public:
  ...
  async_simple::coro::Lazy<resp_data> write_websocket(
    ...
    async_simple::coro::Lazy<resp_data> write_websocket_close(std::string msg =
                                                                  "") {
    ...
    async_simple::coro::Lazy<resp_data> async_upload_multipart(
      ...
      if (!ok) {
        co_return resp_data{std::make_error_code(std::errc::protocol_error),
                            404};
      }
    ...
    async_simple::coro::Lazy<resp_data> async_download(
        std::string uri, std::string filename, std::string range = "") {
      resp_data data{};
    ...
    resp_data download(std::string uri, std::string filename,
                       std::string range = "") {
      return async_simple::coro::syncAwait(async_download(
          std::move(uri), std::move(filename), std::move(range)));
    }
   ...
   private:
    ...
    async_simple::coro::Lazy<resp_data> handle_read(
        ...
        req_context<String> ctx, http_method method) {
      resp_data data{};
      do {
    ...
    async_simple::coro::Lazy<void> handle_entire_content(
        resp_data & data, size_t content_len, bool is_ranges, auto &ctx) {
      if (content_len > 0) {
        const char *data_ptr;
        if (head_buf_.size() == 0) {
          if (out_buf_.empty()) {
    ...
    void handle_result(resp_data & data, std::error_code ec,
                       bool is_keep_alive) {
      if (ec) {
        close_socket(*socket_);
        data.net_err = ec;
    ...
    async_simple::coro::Lazy<resp_data> send_single_part(
      ...
      if (is_file) {
        ...
        while (true) {
          ...
          if (file.eof()) {
            if (auto [ec, size] = co_await async_write(asio::buffer(CRCF));
                ec) {
              co_return resp_data{ec, 404};
            }
            break;
    ...
    async_simple::coro::Lazy<resp_data> async_read_ws() {
      ...
      while (true) {
        ...
        if (is_close_frame) {
          ...
          auto close_str = ws.format_close_payload(
              close_code::normal, reason.data(), reason.size());
          auto span = std::span<char>(close_str);
          std::string encode_header =
              ws.encode_frame(span, opcode::close, true);
    ...
    async_read_until(AsioBuffer & buffer, asio::string_view delim) noexcept {
#ifdef CINATRA_ENABLE_SSL
      if (has_init_ssl_) {
