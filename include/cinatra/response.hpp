...
public:
  ...
  void
  set_status_and_content(status_type status, std::string &&content,
                         std::string &&res_content_type_str,
                         content_encoding encoding = content_encoding::none) {
    status_ = status;
  ...
  void handle_render_view(const std::string &tpl_file_path,
                          const nlohmann::json &tmp_data,
                          status_type server_type = status_type::ok) {
    std::string res_content_type_str = get_res_content_type_str(tpl_file_path);
#ifdef CINATRA_ENABLE_GZIP
    set_status_and_content(server_type, env.render_template(tmpl, tmp_data),
                           std::move(res_content_type_str),
                           content_encoding::gzip);
#else
    set_status_and_content(
        server_type, render::render_file(tpl_file_path, tmp_data),
        std::move(res_content_type_str), content_encoding::none);
#endif
  }
  ...
  void render_raw_view(const std::string &file_path,
                       status_type server_type = status_type::ok) {
    std::string res_content_type_str = get_res_content_type_str(file_path);
#ifdef CINATRA_ENABLE_GZIP
    set_status_and_content(server_type, env.render_template(file_path),
                           std::move(res_content_type_str),
                           content_encoding::gzip);
#else
    set_status_and_content(server_type, render::render_file(file_path),
                           std::move(res_content_type_str),
...
#ifdef CINATRA_ENABLE_GZIP
    set_status_and_content(status_type::ok, json_data.dump(),
                           res_content_type::json, content_encoding::gzip);
#else
    set_status_and_content(status_type::ok, json_data.dump(),
                           res_content_type::json, content_encoding::none);
#endif
  }
...
#else
    set_status_and_content(status_type::ok, std::move(content),
                           res_content_type::string, content_encoding::none);
#endif
  }
...
#ifdef CINATRA_ENABLE_GZIP
    set_status_and_content(status_type::not_found,
                           std::string(not_found.data(), not_found.size()),
                           res_content_type::html, content_encoding::gzip);
#else
    set_status_and_content(status_type::not_found,
                           std::string(not_found.data(), not_found.size()),
                           res_content_type::html, content_encoding::none);
#endif
  }
  ...
  template <typename T> void set_attr(const std::string &key, const T &value) {
  ...
  static int get_counter() { return counter_; }

  static void increase_counter() { counter_++; }
