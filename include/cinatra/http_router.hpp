...
public:
  ...
  register_handler(std::string_view name, Function &&f, const Ap &... ap) {
    ...
    if (name.back() == '*') {
      memento::pathinfo_mem.push_back(
          std::string_view(name.data(), name.length() - 1));
    }
    ...
    if constexpr (sizeof...(Is) > 0) {
      ...
      register_nonmember_func(name, std::string(name.data(), name.length()),
...
private:
  bool get_wildcard_function(const std::string &key, request &req,
                             response &res) {
  ...
  void register_nonmember_func(std::string_view raw_name,
    ...
    if (raw_name.back() == '*') {
      this->wildcard_invokers_[name.substr(0, name.size() - 2)] = std::bind(
          &http_router::invoke<Function, AP...>, this, std::placeholders::_1,
          std::placeholders::_2, std::move(f), ap...);
    } else {
  ...
  void register_member_func(std::string_view raw_name, const std::string &name,
  ...
  std::unordered_map<std::string, invoker_function>
