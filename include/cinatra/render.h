...
class object {
  ...
  template <class T> struct holder_impl : holder {
    T obj;
    ...
    template <class> bool cond_(...) const {
    ...
    template <typename Object> std::string str_help(Object &t) const {
      std::stringstream ss;
      ss << t;
      return ss.str();
    }
    ...
    template <class U> std::string str_(...) const {
    ...
    template <class U> object get_(long, std::string, ...) {
      throw "This value does not have operator[]().";
    }
...
public:
  ...
  template <class T> object(T v) : holder_(new holder_impl<T>(std::move(v))) {}
...
template <class Iterator> class parser {
public:
  Iterator current_;
  Iterator next_;
  Iterator last_;
...
public:
  ...
  template <class F> range_t read_while(F f) {
    if (current_ == last_)
  ...
  template <class F> range_t read_while_or_eof(F f) {
    auto first = current_;
...
template <class IOS> struct output_type {
  IOS ios;
  output_type(IOS ios) : ios(std::forward<IOS>(ios)) {}

  template <class Iterator> void put(Iterator first, Iterator last) {
...
template <class IOS> static output_type<IOS> from_ios(IOS &&ios) {
...
struct cstring : std::iterator<std::forward_iterator_tag, char> {
  ...
  friend bool operator==(const cstring &a, const cstring &b) {
    return not a.p && not b.p
               ? true
               : not a.p && b.p ? *b.p == '\0'
                                : a.p && not b.p ? *a.p == '\0' : a.p == b.p;
  }
  friend bool operator!=(const cstring &a, const cstring &b) {
    return !(a == b);
  }
