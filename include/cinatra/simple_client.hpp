...
public:
#ifdef CINATRA_ENABLE_SSL
  simple_client(boost::asio::io_service &io_context, std::string addr,
                std::string port, boost::asio::ssl::context &context,
                size_t timeout = 30)
      : ios_(io_context), socket_(io_context, context), resolver_(io_context),
        addr_(std::move(addr)), port_(std::move(port)), timer_(io_context),
        timeout_seconds_(timeout), chunked_size_buf_(10) {
    socket_.set_verify_mode(boost::asio::ssl::verify_peer);
    socket_.set_verify_callback(
        [this](auto b, auto &ctx) { return verify_certificate(b, ctx); });
    chunk_body_.resize(chunk_buf_len + 4);
  }
#else
  simple_client(boost::asio::io_service &io_context, std::string addr,
                std::string port, size_t timeout = 30)
      : ios_(io_context), socket_(io_context), resolver_(io_context),
        addr_(std::move(addr)), port_(std::move(port)), timer_(io_context),
        timeout_seconds_(timeout), chunked_size_buf_(10) {
    chunk_body_.resize(chunk_buf_len + 4);
  }
  ...
  template <res_content_type CONTENT_TYPE = res_content_type::json,
            size_t TIMEOUT = 3000, http_method METHOD = POST>
  ...
  template <res_content_type CONTENT_TYPE = res_content_type::json,
            size_t TIMEOUT = 3000, http_method METHOD = POST>
  void async_send_msg(
      std::string api, std::string msg,
      std::function<void()> error_callback = [] {}) {
    build_message<METHOD, CONTENT_TYPE>(std::move(api), msg);
    boost::asio::ip::tcp::resolver::query query(addr_, port_);
    auto self = this->shared_from_this();
    resolver_.async_resolve(
        query, [this, self, callback = std::move(error_callback)](
  ...
  void set_url_preifx(std::string prefix) { prefix_ = std::move(prefix); }

  // set http version to 1.0, default 1.1
  void set_version() { version_ = " HTTP/1.0\r\n"; }

  std::string_view get_header_value(std::string_view key) {
    return parser_.get_header_value(key);
  }

  void send_form_data(std::string api,
                      std::vector<std::pair<std::string, std::string>> v,
                      std::function<void()> error_callback) {
    build_form_data(std::move(v));

    prefix_ = build_head<http_method::POST, res_content_type::multipart>(
        api, total_multipart_size());
    write_message_ = std::move(prefix_) + std::move(multipart_start_) +
                     std::move(multipart_end_);
  ...
  void upload_file(
      std::string api, std::string filename,
      std::function<void(boost::system::error_code ec)> error_callback) {
    upload_file(std::move(api), std::move(filename), 0,
                std::move(error_callback));
  }

  void upload_file(
      std::string api, std::string filename, size_t start,
      std::function<void(boost::system::error_code ec)> error_callback) {
    if (file_.is_open()) {
      error_callback(
    ...
    if (!file_) {
      error_callback(boost::asio::error::make_error_code(
  ...
  void download_file(
      std::string filename, std::string resoure_path,
      std::function<void(boost::system::error_code ec)> error_callback) {
    download_file<METHOD>("", std::move(filename), std::move(resoure_path),
                          std::move(error_callback));
  }

  template <http_method METHOD = GET>
  void download_file(
      std::string dir, std::string filename, std::string resoure_path,
      std::function<void(boost::system::error_code ec)> error_callback) {
...
private:
  ...
  void build_message(std::string api, std::string msg) {
    std::string prefix =
        build_head<METHOD, CONTENT_TYPE>(std::move(api), msg.size());
  ...
  std::string build_head(std::string api, size_t content_length) {
    ...
    std::string prefix = method.append(" ")
                             .append(std::move(prefix_))
                             .append(std::move(api))
                             .append(std::move(version_));
  ...
  void multipart_file_start() {
    ...
    multipart_start_.append("Content-Disposition: form-data; name=\"" +
                            std::string("test") + "\"; filename=\"" +
                            std::string("filename") + file_extension_ + "\"" +
                            CRLF);
  ...
  void multipart_end(std::string &body) { body.append(multipart_end_); }
  ...
  template <http_method METHOD> std::string get_method_str() {
    return std::string{method_name(METHOD)};
  }

  template <res_content_type CONTENT_TYPE> void build_content_type() {
  ...
  void do_write(std::function<void()> error_callback = [] {}) {
    ...
    boost::asio::async_write(
        socket_,
        boost::asio::buffer(write_message_.data(), write_message_.length()),
        [this, self, error_callback = std::move(error_callback)](
  ...
  void do_read() {
    ...
    socket_.async_read_some(
        boost::asio::buffer(parser_.buffer(), parser_.left_size()),
        [this, self](boost::system::error_code ec,
          ...
          if (!ec) {
            ...
            if (ret == -2) {
              ...
              if (parser_.total_len() > MAX_RESPONSE_SIZE) {
                set_response_msg("response message too long, more than " +
  ...
  void do_read_body() {
    ...
    boost::asio::async_read(
        socket_,
        boost::asio::buffer(parser_.buffer(),
                            parser_.total_len() - parser_.current_size()),
  ...
  void
  read_chunk(std::function<void(boost::system::error_code ec)> error_callback) {
    reset_timer();
    auto self = this->shared_from_this();
    boost::asio::async_read_until(
        socket_, read_head_, "\r\n\r\n",
        [this, self, callback = std::move(error_callback)](
          ...
          std::string_view line(boost::asio::buffer_cast<const char *>(bufs),
                                bytes_transferred);
          ...
          std::string_view part_body(
              boost::asio::buffer_cast<const char *>(bufs) + bytes_transferred,
              boost::asio::buffer_size(bufs) - bytes_transferred);
    ...
    if (dec._state == 0) {
      ...
      while (true) {
        ...
        if (left_chunk_len_ == left) {
          ...
          if (left_chunk_len_ >= chunk_buf_len) {
            read_chunk_body(chunk_buf_len + 2, false,
                            std::move(error_callback));
          } else {
            read_chunk_body(left_chunk_len_ + 2, false,
                            std::move(error_callback));
          }
          break;
        } else if (left_chunk_len_ < left) {
          write_chunked_data(
              {content.data() + pos + 2, (size_t)left_chunk_len_});
          ...
          if (len == 1) {
            ...
            content = {content.data() + left_chunk_len_ + 2,
                       (size_t)(content.size() - left_chunk_len_ - 2)};
          }
        }
      }
  ...
  void
  read_crcf(size_t count,
            std::function<void(boost::system::error_code ec)> error_callback) {
    auto self = this->shared_from_this();
    boost::asio::async_read(
        socket_, boost::asio::buffer(crcf_, count),
        [this, self, callback = std::move(error_callback)](
            boost::system::error_code ec, std::size_t length) {
    ...
    boost::asio::async_read_until(
        ...
        [this, self, callback = std::move(error_callback)](
          ...
          boost::asio::streambuf::const_buffers_type bufs =
              chunked_size_buf_.data();
          std::string line(boost::asio::buffer_cast<const char *>(bufs),
                           bytes_transferred - 2);
          ...
          if (part_body.size() > left_chunk_len_) {
            ...
            std::string_view left_data(part_body.data() + left_chunk_len_ + 2,
                                       part_body.length() - left_chunk_len_ -
                                           2);
            if (left_data.size() == 5) { //"\r\n0\r\n"
              chunked_file_.close();
              callback({});
  ...
  void read_chunk_body(
      int64_t read_len, bool eof,
      std::function<void(boost::system::error_code ec)> error_callback) {
    reset_timer();
    auto self = this->shared_from_this();
    boost::asio::async_read(
        socket_, boost::asio::buffer(chunk_body_.data(), read_len),
        [this, eof, self, callback = std::move(error_callback)](
            boost::system::error_code ec, std::size_t length) {
          if (ec) {
            chunked_file_.close();
            return;
