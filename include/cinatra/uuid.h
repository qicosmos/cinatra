...
struct uuid {
  ...
  public:
    ...
    self_type &operator++() {
      if (index >= uuid::state_size)
        throw std::out_of_range(
            "Iterator cannot be incremented past the end of the data.");
      ++index;
      return *this;
    }
    ...
    self_type &operator--() {
      if (index <= 0)
        throw std::out_of_range(
            "Iterator cannot be decremented past the beginning of the data.");
      --index;
      return *this;
    }
...
public:
  ...
  explicit uuid(std::string_view str) { create(str.data(), str.size()); }
  ...
  std::string to_short_str() {
    std::stringstream s;
    s << std::hex << std::setfill(static_cast<char>('0')) << std::setw(2)
      << (int)data[0] << std::setw(2) << (int)data[1] << std::setw(2)
      << (int)data[2] << std::setw(2) << (int)data[3] << std::setw(2)
      << (int)data[4] << std::setw(2) << (int)data[5] << std::setw(2)
      << (int)data[6] << std::setw(2) << (int)data[7] << std::setw(2)
      << (int)data[8] << std::setw(2) << (int)data[9] << std::setw(2)
      << (int)data[10] << std::setw(2) << (int)data[11] << std::setw(2)
      << (int)data[12] << std::setw(2) << (int)data[13] << std::setw(2)
      << (int)data[14] << std::setw(2) << (int)data[15];
    return s.str();
  }

  iterator begin() noexcept { return uuid_iterator(&data[0], 0); }
  const_iterator begin() const noexcept {
    return uuid_const_iterator(&data[0], 0);
  }
  iterator end() noexcept { return uuid_iterator(&data[0], state_size); }
  const_iterator end() const noexcept {
    return uuid_const_iterator(&data[0], state_size);
  }
...
inline void swap(uuids::uuid &lhs, uuids::uuid &rhs) { lhs.swap(rhs); }
