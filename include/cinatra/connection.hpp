...
namespace cinatra {
  ...
  public:
    ...
    void write_ranges_header(std::string header_str) {
      ...
      boost::asio::async_write(socket(),
        boost::asio::buffer(chunked_header_),
        [this, self = this->shared_from_this()](const boost::system::error_code& ec, std::size_t) {
  ...
  private:
    ...
    void handle_pipeline(size_t ret, std::size_t bytes_transferred) {
      ...
      boost::asio::async_write(socket(), boost::asio::buffer(rep_str.data(), rep_str.size()),
        ...
        self = this->shared_from_this(), &rep_str](const boost::system::error_code& ec, std::size_t) {
        rep_str.clear();
...
//			}
    ...
    void response_handshake() {
      ...
      auto self = this->shared_from_this();
      boost::asio::async_write(socket(), buffers, [this, self](const boost::system::error_code& ec, std::size_t) {
        if (ec) {
          close();
          return;
        }
    ...
    void do_write_msg() {
      ...
      boost::asio::async_write(socket(), buffer_seq_, [this, self = this->shared_from_this()](const boost::system::error_code& ec, size_t) {
        std::lock_guard<std::mutex> lock(buffers_mtx_);
