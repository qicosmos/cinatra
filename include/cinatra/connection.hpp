...
public:
  explicit connection(
      boost::asio::io_service &io_service, std::size_t max_req_size,
      ...
      :
        ...
        timer_(io_service), http_handler_(handler), req_(this, res_),
        static_dir_(static_dir), upload_check_(upload_check) {
  ...
  void write_chunked_header(std::string_view mime, bool is_range = false) {
    ...
    boost::asio::async_write(
        socket_, boost::asio::buffer(chunked_header_),
...
private:
  ...
  void handle_read(const boost::system::error_code &e,
    ...
    if (ret == parse_status::has_error) { // 4.1 parse request error, response
                                          // bad request
      response_back(status_type::bad_request);
      return;
    }
    ...
    if (ret == parse_status::not_complete) { // 4.2 not completed, continue read
      ...
      if (req_.get_method() == "GET" &&
          http_cache::get().need_cache(req_.get_url()) &&
          !http_cache::get().not_cache(req_.get_url())) {
        auto raw_url = req_.raw_url();
        if (!http_cache::get().empty()) {
          auto resp_vec = http_cache::get().get(
              std::string(raw_url.data(), raw_url.length()));
          // write back cache
          if (!resp_vec.empty()) {
            std::vector<boost::asio::const_buffer> buffers;
            for (auto &iter : resp_vec) {
              buffers.emplace_back(
                  boost::asio::buffer(iter.data(), iter.size()));
            }
            boost::asio::async_write(
                socket_, buffers,
                [self = this->shared_from_this(),
                 resp_vec = std::move(resp_vec)](
      ...
      } else { // 5.2 just head, no body; because websocket just 'GET' head, no
               // body, so need deal with websocket
  ...
  void do_read_body() {
    ...
    boost::asio::async_read(
        socket_, boost::asio::buffer(req_.buffer(), req_.left_body_len()),
  ...
  void handle_octet_stream(size_t bytes_transferred) {
    ...
    try {
      std::string name =
          static_dir_ + uuids::uuid_system_generator{}().to_short_str();
  ...
  void init_multipart_parser() {
    multipart_parser_.on_part_begin = [this](const multipart_headers &headers) {
      ...
      if (is_multi_part_file_) {
        ...
        try {
          std::string name = static_dir_ +
                             uuids::uuid_system_generator{}().to_short_str() +
  ...
  void do_read_part_data() {
    auto self = this->shared_from_this();
    boost::asio::async_read(
        socket_, boost::asio::buffer(req_.buffer(), req_.left_body_size()),
        [self, this](boost::system::error_code ec, std::size_t length) {
          if (ec) {
            req_.set_state(data_proc_state::data_error);
            call_back();
  ...
  void do_read_websocket_head(size_t length) {
    ...
    boost::asio::async_read(socket_, boost::asio::buffer(req_.buffer(), length),
                            [this, self](const boost::system::error_code &ec,
                              ...
                              size_t length =
                                  bytes_transferred + req_.current_size();
                              req_.set_current_size(0);
  ...
  void do_read_websocket_data(size_t length) {
    auto self = this->shared_from_this();
    boost::asio::async_read(
        socket_, boost::asio::buffer(req_.buffer(), length),
        [this, self](const boost::system::error_code &ec,
                     size_t bytes_transferred) {
          if (ec) {
            req_.call_event(data_proc_state::data_error);
