...
public:
  ...
  void on_data(callback_t cb) { cb_ = std::move(cb); }

  void
  async_request(std::string_view method, std::string_view url,
...
private:
  void async_connect(const std::string &host, const std::string &port,
    ...
    resolver_.async_resolve(
        query, [this, self = this->shared_from_this(),
  ...
  void do_read() {
    async_read_until(
        TWO_CRCF, [this, self = shared_from_this()](auto ec, size_t size) {
          if (ec) {
            close();
            return;
          }
  ...
  void read_chunk_body(bool keep_alive, size_t size_to_read) {
    async_read(size_to_read, [this, self = shared_from_this(),
                              keep_alive](auto ec, size_t size) {
      if (!ec) {
        if (size <= CRCF.size()) {
          // finish all chunked
  ...
  void append_chunk(std::string_view chunk) { chunked_result_.append(chunk); }
