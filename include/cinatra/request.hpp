...
template <typename socket_type> class connection;
...
public:
  ...
  auto get_conn() const { return con_; }
  ...
  std::string_view get_query_value(size_t n) {
    ...
    for (auto item : memento::pathinfo_mem) {
      if (url.find(item) != std::string_view::npos) {
        ...
        auto str =
            url.substr(item.length(), url.length() - item.length() - tail);
        auto params = split(str, "/");
        if (n >= params.size())
          return {};
        if (code_utils::is_url_encode(params[n])) {
          auto map_url = url.length() > 1 && url.back() == '/'
                             ? url.substr(0, url.length() - 1)
                             : url;
          std::string map_key =
              std::string(map_url.data(), map_url.size()) + std::to_string(n);

          auto ret = utf8_character_pathinfo_params_.emplace(
              map_key, code_utils::get_string_by_urldecode(params[n]));
