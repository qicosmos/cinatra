...
TEST_CASE("test websocket with different message sizes") {
    ...
    server.set_http_handler<cinatra::GET>(
        "/ws_echo",
        [](cinatra::coro_http_request& req, cinatra::coro_http_response& resp) -> async_simple::coro::Lazy<void> {
    ...
    SUBCASE("medium message - 16 bit length") {
        std::string medium_message(65535, 'x'); // 65,535 'x' characters for the medium message test.
        ...
        async_simple::coro::syncAwait(client->async_ws_connect("ws://127.0.0.1:9001/ws_echo"));
        async_simple::coro::syncAwait(client->async_send_ws(medium_message));
        async_simple::coro::syncAwait(client->async_send_ws_close());
    }

    SUBCASE("large message - 64 bit length") {
        std::string large_message(70000, 'x'); // 70,000 'x' characters for the large message test.
