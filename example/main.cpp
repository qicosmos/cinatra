...
int main() {
  ...
  //    },enable_cache{false});
  //
  //	server.set_http_handler<GET, POST>("/login", [](request& req, response& res) { 		auto session = res.start_session(); 		session->set_data("userid", std::string("1")); 		session->set_max_age(-1);
  //		res.set_status_and_content(status_type::ok, "login");
  //	},enable_cache{false});
  //
  //	server.set_http_handler<GET, POST>("/islogin", [](request& req, response& res) { 		auto ptr = req.get_session(); 		auto session = ptr.lock(); 		if (session == nullptr || session->get_data<std::string>("userid") != "1") {
  //			res.set_status_and_content(status_type::ok, "没有登录", res_content_type::string); 			return;
  //		}
  ...
  //	});
  //
  //	server.set_http_handler<GET, POST,OPTIONS>("/json", [](request& req, response& res) { 		nlohmann::json json;
  ...
  //	});
  //
  //	server.set_http_handler<GET,POST>("/redirect",[](request& req, response& res){ 		res.redirect("http://www.baidu.com"); // res.redirect("/json");
  //	});
  //
  //	server.set_http_handler<GET, POST>("/pathinfo/*", [](request& req, response& res) { 		auto s = req.get_query_value(0);
  //		res.render_string(std::string(s.data(), s.length()));
  //	});
  //
  //	server.set_http_handler<GET, POST>("/restype", [](request& req, response& res) { 		auto type = req.get_query_value("type"); 		auto res_type = cinatra::res_content_type::string; 		if (type == "html")
  //		{
  ...
  //	});
  //
  //	server.set_http_handler<GET, POST>("/getzh", [](request& req, response& res) { 		auto zh = req.get_query_value("zh");
  //		res.render_string(std::string(zh.data(),zh.size()));
  //	});
  //
  //	server.set_http_handler<GET, POST>("/gzip", [](request& req, response& res) { 		auto body = req.body(); 		std::cout << body.data() << std::endl;
  //		res.set_status_and_content(status_type::ok, "hello world", res_content_type::none, content_encoding::gzip);
  //	});
  //
  //
  //	server.set_http_handler<GET, POST>("/test", [](request& req, response& res) { 		auto name = req.get_header_value("name"); 		if (name.empty()) {
  ...
  //
  //	//aspect
  //	server.set_http_handler<GET, POST>("/aspect", [](request& req, response& res) { 		res.render_string("hello world");
  //	}, check{}, log_t{});
  //
  ////web socket
  ...
  //		//echo
  //		std::string str = std::string(part_data.data(), part_data.length()); 		req.get_conn()->send_ws_string(std::move(str));
  ...
  //	server.set_http_handler<GET, POST>("/vue_html", [](request& req, response& res) { 		res.render_raw_view("./www/index.html");
  //	});
  //
  //	server.set_http_handler<GET, POST>("/vue_demo", [](request& req, response& res) { 		res.render_raw_view("./www/dist/index.html");
  //	});
  //
  //	//http upload(multipart)
  // server.set_http_handler<GET, POST>("/upload_multipart", [](request& req, response& res) { 	assert(req.get_content_type() == content_type::multipart);
  ...
  //	server.set_http_handler<GET, POST>("/upload_octet_stream", [](request& req, response& res) { 		assert(req.get_content_type() == content_type::octet_stream); 		auto& files = req.get_upload_files(); 		for (auto& file : files) { 			std::cout << file.get_file_path() << " " << file.get_file_size() << std::endl;
  //		}
