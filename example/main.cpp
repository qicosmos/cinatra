...
int main() {
  ...
  server.init_ssl_context(
      true, [](auto, auto) { return "123456"; }, "server.crt", "server.key",
      "dh1024.pem");
  ...
  server.set_http_handler<GET, POST>(
      "/404", [](request &req, response &res) { res.render_404(); },
      enable_cache{false});

  server.set_http_handler<GET, POST>(
      "/404_custom",
      [](request &req, response &res) { res.render_404("./404.html"); },
      enable_cache{false});
  ...
  server.set_http_handler<GET, POST, OPTIONS>(
  ...
  server.set_http_handler<GET, POST>(
      "/aspect",
      [](request &req, response &res) { res.render_string("hello world"); },
      check{}, log_t{});
  ...
  server.set_http_handler<GET, POST>("/ws", [](request &req, response &res) {
    ...
    req.on(ws_close,
           [](request &req) { std::cout << "websocket close" << std::endl; });

    req.on(ws_error,
           [](request &req) { std::cout << "websocket error" << std::endl; });
  });

  server.set_http_handler<GET, POST>("/vue_html",
                                     [](request &req, response &res) {
                                       res.render_raw_view("./www/index.html");
                                     });

  server.set_http_handler<GET, POST>(
      "/vue_demo", [](request &req, response &res) {
  ...
  server.set_http_handler<GET, POST>(
      "/upload_octet_stream", [](request &req, response &res) {
