...
namespace cinatra {
	...
	class http_cache {
	public:
		static void add(const std::string& key, const std::string& content) {
	...
	private:
		...
		static std::unordered_map<std::string, std::string>::iterator cur_it_;
		static std::unordered_set<std::string_view> skip_cache_;
	};

	std::unordered_map<std::string, std::string> http_cache::cache_;
	std::unordered_map<std::string, std::string>::iterator http_cache::cur_it_= http_cache::cache_.begin();
